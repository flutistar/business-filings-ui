...
import { PageSizes, PAGE_SIZE_DICT, DOCUMENT_TYPES } from '@/enums'
...
export default class FileUpload extends Vue {
  ...
  @Prop({ required: true }) readonly businessIdentifier!: string


  @Prop({ required: true })
  readonly uploadDocumentToDRS!: (
    document: File,
  ...
  async uploadFile (file: File): Promise<string> {
    try {
      ...
      if (res?.status === 200 || res?.status === 201) {
        return res.data.documentServiceId
      }
      throw new Error()
    } catch (err) {
